...
<head>
    ...
    <style>
        * { font-family: 'Sora', sans-serif; box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        /* ═══ CARD STACK ═══ */
        .card-stack { position: relative; width: 100%; max-width: 340px; aspect-ratio: 3/4.2; margin: 0 auto; }
        ...
        .swipe-card.card-behind-1, .swipe-card.card-behind-2 {
        ...
        /* Swipe stamps */
        .stamp { position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%) scale(0.5); opacity: 0; transition: opacity 0.15s, transform 0.15s; pointer-events: none; z-index: 10; }
        .stamp-like { color: #22c55e; border: 4px solid #22c55e; }
        .stamp-nope { color: #ef4444; border: 4px solid #ef4444; }
        .stamp.visible { opacity: 1; transform: translate(-50%,-50%) scale(1); }
        ...
        .action-btn {
            ...
            transition: all 0.2s cubic-bezier(0.22,1,0.36,1); flex-shrink: 0;
            box-shadow: 0 4px 20px rgba(0,0,0,0.3);
        }
        .action-btn:active { transform: scale(0.88); }
        .action-btn-pass { background: rgba(239,68,68,0.12); border: 2px solid rgba(239,68,68,0.25); }
        .action-btn-pass:active { background: rgba(239,68,68,0.25); }
        .action-btn-like { background: rgba(34,197,94,0.12); border: 2px solid rgba(34,197,94,0.25); }
        .action-btn-like:active { background: rgba(34,197,94,0.25); }
        .action-btn-filter { background: rgba(255,255,255,0.06); border: 2px solid rgba(255,255,255,0.08); width: 44px; height: 44px; }
        ...
        .match-pop { animation: matchPop 0.5s cubic-bezier(0.22,1,0.36,1) both; }
        ...
        .modal-content { transition: transform 0.3s cubic-bezier(0.22,1,0.36,1), opacity 0.3s ease; }
...
<?php endif; ?>
        ...
        <div class="modal-content relative w-full max-w-md mx-4 mb-4 sm:mb-0 rounded-3xl p-6" style="background: linear-gradient(135deg, rgba(26,17,69,0.98), rgba(15,26,58,0.98)); border: 1px solid rgba(255,255,255,0.08);">
            <div class="flex items-center justify-between mb-6">
                ...
                <button onclick="closeFilter()" class="p-2 rounded-xl hover:bg-white/5 transition">
                    <svg class="w-5 h-5 text-white/30" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            <div class="space-y-4">
                <div>
    ...
    let startX = 0, startY = 0, currentDragX = 0;
    let activeCard = null;
    ...
    function onDragEnd() {
        ...
        if (currentDragX > 100) {
            ...
            // Reset behind cards
            cardStack.querySelectorAll('.card-behind-1').forEach(c => { c.style.transform = ''; c.style.opacity = ''; c.style.filter = ''; });
            cardStack.querySelectorAll('.card-behind-2').forEach(c => { c.style.transform = ''; c.style.opacity = ''; c.style.filter = ''; });
        }
    }
    ...
    async function swipe(direction) {
        ...
        if (direction === 'right') {
            try {
                const res = await fetch('/like/' + profile.id, { method: 'POST', headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' } });
            ...
            fetch('/pass/' + profile.id, { method: 'POST', headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' } }).catch(() => {});
        }
