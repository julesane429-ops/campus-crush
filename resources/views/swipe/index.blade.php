...
<head>
    ...
    <style>
        * {
        ...
        .match-photo-left {
            animation: photoSlideLeft 0.55s cubic-bezier(0.22, 1, 0.36, 1) 0.2s both;
        }

        .match-photo-right {
            animation: photoSlideRight 0.55s cubic-bezier(0.22, 1, 0.36, 1) 0.2s both;
        }

        .match-heart {
            animation: heartBounce 0.6s cubic-bezier(0.22, 1, 0.36, 1) 0.5s both;
        }

        .match-title {
            animation: titleSlide 0.5s cubic-bezier(0.22, 1, 0.36, 1) 0.65s both;
        }

        .match-btns {
            animation: titleSlide 0.5s cubic-bezier(0.22, 1, 0.36, 1) 0.8s both;
        }
...
<body class="h-full text-white overflow-hidden">
    ...
    <div id="swipe-tutorial"
        ...
        <p class="text-[11px] text-white/15 mt-4">Appuie n'importe où pour fermer</p>
    </div>
...
${profile.badge === 'queen'
    ...
    : ''}
        ...
        // ═══════════════════════════════════════
        // TUTORIAL — premier swipe uniquement
        // ═══════════════════════════════════════
        const TUTORIAL_KEY = 'cc_swipe_tutorial_v1';

        function showTutorial() {
            if (localStorage.getItem(TUTORIAL_KEY)) return; // déjà vu
            if (profiles.length === 0) return; // rien à swiper

            const overlay = document.getElementById('swipe-tutorial');
        ...
        // Montrer le tutorial 800ms après le chargement
        // (le temps que l'utilisateur voit la carte)
