...
<body class="flex items-center justify-center text-white">
    ...
    <div class="relative z-10 w-full max-w-sm px-5 py-6" style="padding-top: max(env(safe-area-inset-top, 24px), 24px);">
        ...
        <div class="text-center mb-8 fade-up">
            <div class="logo-pulse inline-flex w-16 h-16 rounded-3xl items-center justify-center mb-3" style="background: linear-gradient(135deg, rgba(255,94,108,0.15), rgba(255,193,69,0.1)); border: 1px solid rgba(255,94,108,0.15);">
                <svg class="w-8 h-8" fill="url(#lg)" viewBox="0 0 24 24">
        ...
        <div class="rounded-3xl p-5 sm:p-7 fade-up d1" style="background: linear-gradient(135deg, rgba(255,255,255,0.05), rgba(255,255,255,0.02)); border: 1px solid rgba(255,255,255,0.08); backdrop-filter: blur(40px);">
            <h2 class="text-lg font-semibold text-center mb-5">Connexion</h2>

            <form method="POST" action="{{ route('login') }}" class="space-y-3.5">
                @csrf
                <div>
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="Email" required class="cc-input" autocomplete="email">
                    @error('email')<p class="text-red-400 text-xs mt-1.5">{{ $message }}</p>@enderror
                </div>
                <div class="relative">
                    <input type="password" name="password" id="pwd" placeholder="Mot de passe" required class="cc-input pr-12" autocomplete="current-password">
                ...
                <button type="submit" class="w-full py-3.5 rounded-2xl font-semibold text-white text-base transition active:scale-[0.98]" style="background: linear-gradient(135deg, #ff5e6c, #ff8a5c); box-shadow: 0 8px 30px rgba(255,94,108,0.3);">
                    Se connecter
                </button>
            </form>
        </div>
