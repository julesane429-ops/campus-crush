...
<body class="text-white">
    ...
    <div class="min-h-screen w-full overflow-auto pb-28">
        <div class="relative z-10 max-w-md mx-auto px-5">
            ...
            {{-- ═══════════════════════════════ --}}
            @php
            // Calcul du score — chaque critère a un poids
            $criteria = [
            ...
            ['label' => 'Centres d\'intérêt', 'done' => !empty($profile->interests), 'weight' => 20, 'cta' => 'Ajoute tes intérêts', 'route' => route('profile.edit')],
            ['label' => 'Université', 'done' => !empty($profile->university_id ?? $profile->university), 'weight' => 15, 'cta' => 'Indique ton université', 'route' => route('profile.edit')],
            ...
            $barColor = match(true) {
            $score >= 90 => 'linear-gradient(90deg, #22c55e, #4ade80)',
            $score >= 60 => 'linear-gradient(90deg, #ff5e6c, #ffc145)',
            default => 'linear-gradient(90deg, #ff5e6c, #a855f7)',
            };

            $scoreLabel = match(true) {
            $score >= 90 => ['🏆', 'Profil complet !', 'text-green-400'],
            $score >= 60 => ['🔥', 'Bon profil', 'text-[#ffc145]'],
            $score >= 30 => ['⚡', 'Profil à compléter', 'text-[#ff8a5c]'],
            default => ['🌱', 'Profil incomplet', 'text-white/40'],
            };
            @endphp

            @if($score < 100)
                <div class="mb-6 fade-up d2" style="animation-delay: 0.18s;">
                <div class="cc-surface rounded-2xl p-4">

                    {{-- Header --}}
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2">
                            <span class="text-base">{{ $scoreLabel[0] }}</span>
                    ...
                    {{-- Barre --}}
                    <div class="w-full h-2 rounded-full mb-3" style="background: rgba(255,255,255,0.06);">
                        <div class="h-2 rounded-full transition-all duration-700 ease-out"
                            style="width: {{ $score }}%; {{ $barColor }}; box-shadow: 0 0 8px rgba(255,94,108,0.3);">
                        </div>
                    </div>

                    {{-- Critères en mini-pills --}}
                    <div class="flex flex-wrap gap-1.5 mb-3">
                        @foreach($criteria as $c)
                        <span class="flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-medium transition"
                            style="{{ $c['done']
                    ? 'background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.2); color: rgba(74,222,128,0.7);'
                    ...
                    {{-- CTA vers le champ manquant --}}
                    @if($nextMissing)
                    <a href="{{ $nextMissing['route'] }}"
                        class="flex items-center justify-between w-full px-4 py-2.5 rounded-xl transition active:scale-95"
                        ...
                        <svg class="w-3.5 h-3.5 text-[#ff5e6c]/60" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
        ...
        @else
        {{-- Score 100% — petit badge de félicitations --}}
        <div class="mb-6 fade-up d2 flex justify-center" style="animation-delay: 0.18s;">
            <span class="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold text-green-400"
                style="background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.15);">
                🏆 Profil 100% complété · Tu maximises tes chances de match !
            </span>
        </div>
        @endif
