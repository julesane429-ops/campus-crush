...
<head>
    ...
    <style>
        * { font-family: 'Sora', sans-serif; box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        .bubble-sent { background: linear-gradient(135deg, #ff5e6c 0%, #ff8a5c 100%); border-radius: 20px 20px 6px 20px; box-shadow: 0 4px 20px rgba(255,94,108,0.12); }
        .bubble-received { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.08); border-radius: 20px 20px 20px 6px; }
        @keyframes msgSlideIn { from { opacity: 0; transform: translateY(10px) scale(0.97); } to { opacity: 1; transform: none; } }
        .msg-in { animation: msgSlideIn 0.25s cubic-bezier(0.22,1,0.36,1) both; }
        @keyframes typingDot { 0%,60%,100% { transform: translateY(0); opacity: 0.4; } 30% { transform: translateY(-4px); opacity: 1; } }
        .typing-dot { animation: typingDot 1.4s ease-in-out infinite; }
        .typing-dot:nth-child(2) { animation-delay: 0.15s; }
        .typing-dot:nth-child(3) { animation-delay: 0.3s; }
        @keyframes newMsgGlow { 0% { box-shadow: 0 0 0 0 rgba(255,94,108,0.15); } 100% { box-shadow: none; } }
        .new-msg-glow { animation: newMsgGlow 1s ease-out; }
...
channel.listen('.MessageSent', (data) => {
    ...
    if (data.attachments && data.attachments.length) {
        ...
        data.attachments.forEach(a => { content += '<img src="' + a.url + '" class="w-32 h-32 object-cover rounded-xl" loading="lazy">'; });
        content += '</div>';
    }
    ...
    fetch('/messages/' + matchId, { headers: { 'X-CSRF-TOKEN': csrfToken } }).catch(() => {});
});
...
function setOnlineStatus(online) {
    ...
    if (online) statusDot.style.boxShadow = '0 0 6px rgba(74,222,128,0.5)'; else statusDot.style.boxShadow = '';
}
...
document.getElementById('msg-form').addEventListener('submit', function(e) {
    ...
    if (files.length > 0) {
        ...
        files.forEach(f => { imgHtml += '<img src="' + URL.createObjectURL(f) + '" class="w-32 h-32 object-cover rounded-xl" alt="">'; });
        imgHtml += '</div>';
        contentHtml += imgHtml;
    }
    if (contentHtml) {
    ...
    fetch(this.action, { method: 'POST', body: fd, headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' }, credentials: 'same-origin' }).catch(err => console.error('Send error:', err));
    attachInput.value = '';
});
...
document.getElementById('menu-toggle').addEventListener('click', (e) => { e.stopPropagation(); document.getElementById('chat-menu').classList.toggle('hidden'); });
document.addEventListener('click', (e) => { const m = document.getElementById('chat-menu'); if (!m.contains(e.target)) m.classList.add('hidden'); });
...
aInput.onchange = () => {
    ...
    if (!files.length) { aPrev.classList.add('hidden'); return; }
    aPrev.classList.remove('hidden');
    files.forEach((f) => {
        const wrap = document.createElement('div'); wrap.className = 'relative flex-shrink-0';
        const r = new FileReader();
        r.onload = e => { wrap.innerHTML = '<img src="' + e.target.result + '" class="w-16 h-16 object-cover rounded-xl"><button type="button" class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center text-[10px] leading-none shadow-lg" onclick="this.parentElement.remove(); checkPreview();">×</button>'; aPrev.appendChild(wrap); };
        r.readAsDataURL(f);
    });
};
window.checkPreview = () => { if (!aPrev.children.length) { aPrev.classList.add('hidden'); aInput.value = ''; } };

const emojis = ['😊','😂','😍','😎','👍','🎉','❤️','😢','🤔','😜','🥳','🤩','💯','🔥','🙌','😘','🥰','💕','👋','✨','💪','🎶','😭','🥺','😏','🤗','😤','🤣','🫶'];
const picker = document.getElementById('emoji-picker').querySelector('.emoji-grid');
const msgInput = document.getElementById('msg-input');
emojis.forEach(em => { const b = document.createElement('button'); b.type='button'; b.textContent=em; b.className='text-lg p-1 rounded-lg hover:bg-white/10 active:scale-110 transition'; b.onclick=()=>{msgInput.value+=em;msgInput.focus();}; picker.appendChild(b); });
